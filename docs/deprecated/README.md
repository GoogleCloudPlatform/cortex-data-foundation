...
**IMPORTANT: This README file is obsolete and no longer receives updates. For the most current and reliable information, refer to the new documentation at [Google Cloud Cortex Framework documentation](https://cloud.google.com/cortex/docs)**.
...
**Cortex Data Foundation** is the core architectural component of the Cortex Framework reference architecture and provides packaged analytics artifacts which can be automatically deployed for use with [Google Cloud BigQuery](https://cloud.google.com/bigquery).


![Cortex framework](../../images/deprecated/cortex_framework.png)
...
-  [Oracle EBS](../../images/erd_oracle_ebs.png) ([PDF](../../docs/erd_oracle_ebs.pdf))
...
-  [Youtube via Display & Video 360](../../images/erd_dv360.png) ([PDF](../../docs/erd_dv360.pdf))
-  [Google Analytics 4](../../images/erd_ga4.png) ([PDF](../../docs/erd_ga4.pdf))
...
2. Navigate into the previously downloaded folder.
   ...
   ```
   If you are already familiar with the configuration and requirements, you can skip to the build command in section [Execute deployment](#execute-deployment).
...
![structure for parameters](../../images/deprecated/10.png "image_tooltip")
...
* [Integration options for Oracle EBS](./README_Oracle_EBS.md)
...
![K9 sample deployment](../../images/deprecated/k9_datasets.png)
...
![success message in console](../../images/deprecated/1.png "image_tooltip")
...
3. Locate this service account in [IAM](https://console.cloud.google.com/iam-admin/iam):
    ![Cloud build service account in IAM](../../images/deprecated/6.png "Cloud Build service account")

    or

    ![Cloud build compute service account in IAM](../../images/deprecated/cloudbuild_compute_sa.png "Cloud Build Compute service account")
...
The deployment can run through [a service account with impersonation rights](https://cloud.google.com/build/docs/securing-builds/configure-user-specified-service-accounts), by adding the substitution parameter `_BUILD_ACCOUNT`.

```bash
gcloud builds submit --config=./cloudbuild.yaml --substitutions=_GCS_BUCKET=<Bucket for logs>,_BUILD_ACCOUNT='projects/<Your project>/serviceAccounts/<Your build service account>@<Your project>.iam.gserviceaccount.com'
```

This service account will trigger a Cloud Build job that will in turn run specific steps through the Cloud Build service account. This allows a user to trigger a deployment process without direct access to the resources.
...
![Cloud Build Svc account](../../images/deprecated/2.png "image_tooltip")
...
![Authorize impersonation](../../images/deprecated/3.png "image_tooltip")
...
<details>
  <summary> [Optional] Build in a specific region, or with a private worker pool</summary>

The deployment can run through in a specific [region](https://cloud.google.com/build/docs/locations#selecting_the_region), by adding the substitution parameter `_CLOUD_BUILD_REGION` and with `--region` bash parameter.

```bash
gcloud builds submit --config=./cloudbuild.yaml --substitutions=_GCS_BUCKET=<Bucket for logs>,_CLOUD_BUILD_REGION=us-central1 --region=us-central1
```

A [private worker pool](https://cloud.google.com/build/docs/private-pools/run-builds-in-private-pool) could be configured with the substituion parameter `_WORKER_POOL_NAME` with corresponding `_CLOUD_BUILD_REGION` parameter.

```bash
gcloud builds submit --config=./cloudbuild.yaml --substitutions=_GCS_BUCKET=<Bucket for logs>,_WORKER_POOL_NAME='projects/<Your project>/locations/us-central1/workerPools/<Your worker pool name>',_CLOUD_BUILD_REGION=us-central1 --region=us-central1
```
</details>
...
![data mesh structure for lakes zones and catalog](../../images/deprecated/data_mesh1.png)
...
The file contains global configuration, configuration specific to each workload.

![Config JSON](../../images/deprecated/config.json.png)
...
| `deployMarketing`         | Deploy Marketing        | `true`             | Execute the deployment for [Marketing sources](./README_Marketing.md). |
| `deployOracleEBS`         | Deploy Oracle EBS       | `true`             | Execute the deployment for Oracle EBS workload. |
| `deployDataMesh`          | Deploy Data Mesh        | `true`             | Execute the deployment for Data Mesh, see the Data Mesh [README](/docs/deprecated/data_mesh/README.md) for more info. |
...
* [Salesforce Sales Cloud](./README_SFDC.md#deployment-configuration-for-salesforce-sales-cloud)

* [Marketing](./README_Marketing.md#deployment-configuration-for-marketing)

* [Sustainability](./README_Sustainability.md)

* [Oracle EBS](./README_Oracle_EBS.md)
...
Starting with Release 6.0, Cortex now collects telemetry metrics during the deployment process and operationally when the DAGs are executed. This helps the Cortex team better support our customers, however you may opt out of this by setting `"allowTelemetry": false` in the `config.json` file.
...
| Marketing - Youtube via Display & Video 360 | `src/marketing/src/DV360/config/reporting_settings.yaml`     |
| Marketing - Google Analytics 4              | `src/marketing/src/GA4/config/reporting_settings.yaml`       |
| Oracle EBS                                  | `src/OracleEBS/config/reporting_settings.yaml`     |
...
![Click on Logs are available at](../../images/deprecated/logs1.png)
...
## Move the files into the Cloud Composer (Airflow) DAG bucket
...
Tagging your own changes visibly in your fork or clone of a repository together with some deployment and testing automation will be very helpful when performing upgrades. Check out this [guide for upgrade instructions](/docs/deprecated/upgrade_recommendations/upgrade_recommendations.md).
...
* [Oracle EBS Dashboards for Cortex](https://github.com/looker-open-source/block-cortex-oracle-ebs)
...
* [Youtube via Display & Video 360 Dashboards for Cortex](https://github.com/looker-open-source/block-cortex-youtube)
...
![alt_text](../../images/deprecated/16.png "image_tooltip")
...
![Replication with recordstamp and operation flag merged into cdc processed](../../images/deprecated/17.png "image_tooltip")
...
![Replication with system modstamp and id mapped into cortex schema and merged into cdc processed](../../images/deprecated/17b.png "image_tooltip")
...
![alt_text](../../images/deprecated/18.png "image_tooltip")
...
Sustainability  |       N/A                 |       N/A           | `k9_reporting_bq`        |
...
![alt_text](../../images/deprecated/20.png "image_tooltip")
