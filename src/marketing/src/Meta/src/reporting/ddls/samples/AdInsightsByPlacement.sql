...
/* Sample script showing how to calculate KPIs at Ad and Placement level from daily aggregate table.
...
SELECT
  ...
  ) AS frequency, -- non-additive metric
  ...
  SAFE_DIVIDE(
    placement_details.spend,
    SAFE_DIVIDE(placement_details.impressions, 1000)
  ) AS cpm, -- non-additive metric
  SAFE_DIVIDE(
    placement_details.spend,
    SAFE_DIVIDE(placement_details.reach, 1000)
  ) AS cpp, -- non-additive metric
  SAFE_DIVIDE(
    placement_details.link_clicks,
    placement_details.impressions
...
CROSS JOIN UNNEST(ad_agg.placement_details) AS placement_details
