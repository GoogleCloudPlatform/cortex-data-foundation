...
/* Sample script showing how to calculate KPIs at Ad and Platform level from daily aggregate table.
...
SELECT
  ...
  SAFE_DIVIDE(
    platform_details.spend,
    SAFE_DIVIDE(platform_details.impressions, 1000)
  ) AS cpm, -- non-additive metric
  SAFE_DIVIDE(
    platform_details.spend,
    SAFE_DIVIDE(platform_details.reach, 1000)
...
CROSS JOIN UNNEST(ad_agg.platform_details) AS platform_details
