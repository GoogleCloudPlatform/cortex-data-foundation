...
/* Sample script showing how to calculate KPIs at Ad Set level from daily aggregate table.
...
SELECT
  report_date,
  adset_id,
  ARRAY_TO_STRING(ARRAY(SELECT DISTINCT name FROM UNNEST(targeting_audiences) ORDER BY 1), ' | ')
...
FROM
  `{{ project_id_tgt }}.{{ marketing_meta_datasets_reporting }}.AdsetInsightsDailyAgg`
