...
/* Sample script showing how to calculate KPIs at Campaign level from daily aggregate table.
...
SELECT
  ...
  page_likes,
  photo_views,
  SAFE_DIVIDE(spend, video_p95_watched_actions_video_views) AS cost_per_completed_view, -- non-additive metric
  SAFE_DIVIDE(post_engagements, reach) * 100 AS engagement_rate, -- non-additive metric
  SAFE_DIVIDE(video_p95_watched_actions_video_views, impressions) * 100 AS view_through_rate, -- non-additive metric
  (
    link_clicks
    + post_shares
    + post_reactions
...
FROM `{{ project_id_tgt }}.{{ marketing_meta_datasets_reporting }}.CampaignInsightsDailyAgg`
