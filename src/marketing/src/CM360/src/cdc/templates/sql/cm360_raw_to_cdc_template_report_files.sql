 ...
 *
 * CDC Logic
 * Deduplicating source data based on the latest event_time and processing time (recordstamp).
 */
...
USING (
  ...
  QUALIFY ROW_NUMBER() OVER (
      PARTITION BY
        ...
        {%- endfor %}
      ORDER BY `event_time` DESC, `recordstamp` DESC) = 1 -- Order to get latest values for deduplication.
) AS S
ON (
  {%- for identifier in row_identifiers -%}
