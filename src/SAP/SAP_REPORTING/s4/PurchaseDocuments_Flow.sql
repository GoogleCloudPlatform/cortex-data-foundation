...
EKKN AS (
  ...
  FROM `{{ project_id_src }}.{{ dataset_cdc_processed_s4 }}.ekkn` AS EKKN
...
GoodsReceiptStaging AS (
  ...
  FROM `{{ project_id_src }}.{{ dataset_cdc_processed_s4 }}.ekbe`
...
InvoiceReceiptStaging AS (
  ...
  FROM
    `{{ project_id_src }}.{{ dataset_cdc_processed_s4 }}.ekbe`
  WHERE
...
SELECT
  ...
  - COALESCE(
    COALESCE(
      ...
      GoodsReceipt.WRBTR),
    0) AS GrBalanceAmount_NETWR_WRBTR,
  (COALESCE(
    COALESCE(
      EKKN.NETWR * TCURX.currency_fix,
      EKKN.NETWR,
    ...
    - COALESCE(
      COALESCE(
        GoodsReceipt.WRBTR * TCURX.currency_fix,
        GoodsReceipt.WRBTR),
      0)) * CONV.UKURS AS GrBalanceAmountUSD_NETWR_WRBTR,
  ...
  - COALESCE(
    ...
    0) AS IrBalanceAmount_NETWR_REFWR,
  (COALESCE(
    COALESCE(
      EKKN.NETWR * TCURX.currency_fix,
      EKKN.NETWR,
    ...
    - COALESCE(
      COALESCE(
        InvoiceReceipt.REFWR * TCURX.currency_fix,
        InvoiceReceipt.REFWR),
      0)) * CONV.UKURS AS IrBalanceAmountUSD_NETWR_REFWR
