...
FROM `{{ project_id_src }}.{{ dataset_cdc_processed_s4 }}.tcurc` AS tcurc
INNER JOIN
  `{{ project_id_src }}.{{ dataset_cdc_processed_s4 }}.tcurx` AS tcurx ON tcurc.waers = tcurx.currkey
INNER JOIN
  `{{ project_id_src }}.{{ dataset_cdc_processed_s4 }}.tcurt`AS tcurt
