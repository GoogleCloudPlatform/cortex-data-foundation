...
FROM `{{ project_id_src }}.{{ dataset_cdc_processed_s4 }}.t006` AS T006
INNER JOIN `{{ project_id_src }}.{{ dataset_cdc_processed_s4 }}.t006t` AS T006T
  ON T006.MANDT = T006T.MANDT
    AND T006.DIMID = T006T.DIMID
INNER JOIN `{{ project_id_src }}.{{ dataset_cdc_processed_s4 }}.t006a` AS T006A
