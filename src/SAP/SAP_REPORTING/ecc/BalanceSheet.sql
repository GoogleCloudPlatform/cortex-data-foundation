...
WITH
  ...
  CurrencyConversion AS (
    ...
    WHERE Currency.Client_MANDT = '{{ mandt }}'
...
SELECT
  ...
  MAX(
    COALESCE(NodeText.FinancialStatementItemText_TXT45, GLText.GlAccountLongText_TXT50)
  ) AS NodeText,
  --For parent as root node, hierarchy name is printed as parent text.
  MAX(
    IF(FSV.Level = '02', FSV.HierarchyName, ParentText.FinancialStatementItemText_TXT45)
  ) AS ParentText,
...
LEFT JOIN
  ...
  ON
    ...
    AND FSV.HierarchyName = NodeText.FinancialStatementVersion_VERSN
    AND FSV.FinancialStatementItem = NodeText.FinancialStatementItem_ERGSL
    AND NodeText.LanguageKey_SPRAS = LanguageKey.LanguageKey_SPRAS
    -- TextType_TXTYP = 'K' represents text for the node
    AND NodeText.TextType_TXTYP = 'K'
LEFT JOIN
...
LEFT JOIN
  ...
  ON
    ...
    AND FSV.HierarchyName = ParentText.FinancialStatementVersion_VERSN
    AND ParentId.FinancialStatementItem = ParentText.FinancialStatementItem_ERGSL
    AND ParentText.LanguageKey_SPRAS = LanguageKey.LanguageKey_SPRAS
    -- TextType_TXTYP = 'K' represents text for the node
    AND ParentText.TextType_TXTYP = 'K'
