...
WITH
  FSVGLAccounts AS (
    SELECT
      ...
      AccountingDocument.balancesheetaccountindicator AS BalanceSheetAccountIndicator,
      AccountingDocument.placcountindicator AS PLAccountIndicator,
      ...
      FinancialStatementVersion.ergsl AS FinancialStatementItem,
...
SELECT
  ...
  SUM(SUM(AmountInLocalCurrency)) OVER (PARTITION BY
    ...
    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS CumulativeAmountInLocalCurrency
