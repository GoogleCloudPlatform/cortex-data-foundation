...
FROM `{{ project_id_src }}.{{ dataset_cdc_processed_ecc }}.adrc` AS ADRC
INNER JOIN `{{ project_id_src }}.{{ dataset_cdc_processed_ecc }}.adr6` AS ADR6 ON adrc.client = adr6.client AND adrc.addrnumber = adr6.addrnumber
