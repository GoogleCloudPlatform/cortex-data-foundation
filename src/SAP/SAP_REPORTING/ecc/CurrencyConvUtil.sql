(WITH tcurf AS (
  ...
  FROM `{{ project_id_src }}.{{ dataset_cdc_processed_ecc }}.tcurf`
  WHERE kurst = 'M'
    AND mandt = '{{ mandt_ecc }}'
),
...
INNER JOIN `{{ project_id_src }}.{{ dataset_cdc_processed_ecc }}.tcurx` AS x ON curr.fcurr = x.currkey
INNER JOIN `{{ project_id_src }}.{{ dataset_cdc_processed_ecc }}.tcurx` AS y ON curr.tcurr = y.currkey
  AND curr.mandt = '{{ mandt_ecc }}'
)
