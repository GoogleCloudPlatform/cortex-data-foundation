...
BEGIN
  ...
  CREATE OR REPLACE TABLE `{{ project_id_tgt }}.{{ dataset_reporting_tgt }}.profitcenter_flattened`
  (
    mandt STRING,
    setclass STRING,
    subclass STRING,
  ...
  {% if sql_flavour == 'ecc' -%}
    INSERT INTO `{{ project_id_tgt }}.{{ dataset_reporting_tgt }}.profitcenter_flattened`
    ...
    WHILE (level_iteration = max_level) DO --noqa: disable=L003
      ...
      UPDATE `{{ project_id_tgt }}.{{ dataset_reporting_tgt }}.profitcenter_flattened`
      SET level = level_id
    ...
    CROSS JOIN
    (
      SELECT DISTINCT prctr AS node
      FROM `{{ project_id_src }}.{{ dataset_cdc_processed }}.cepc`
      WHERE mandt = '{{ mandt }}'
    ...
    WHERE mandt='{{ mandt }}'
      ...
      AND node IN (
        ...
        (
          SELECT
            DISTINCT prctr AS node
          FROM
            `{{ project_id_src }}.{{ dataset_cdc_processed }}.cepc`
          WHERE
            mandt = '{{ mandt }}'
            AND kokrs = input_subclass
            AND datbi >= '9999-12-31'
  ...
  {% else -%}
    ...
    (
      ...
      SELECT
        ...
        FALSE
      FROM
      `{{ project_id_src }}.{{ dataset_cdc_processed }}.sethanahier0106`
      WHERE
        mandt = '{{ mandt }}'
...
--for profit center flattening based on your requirement
--noqa: disable=L003
CALL `{{ project_id_tgt }}.{{ dataset_reporting_tgt }}.ProfitCenterHrFlattening`(
  {% if sql_flavour == 'ecc' -%}
    '0106', 'C001' -- noqa: L048
  {% else -%}
    '0106', 'A000'
