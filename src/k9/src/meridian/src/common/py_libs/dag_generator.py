...
def generate_str_from_template(template_file_path: Path, **subs: str) -> str:
    """Creates fully resolved string from template using substitutions.
    ...
    return dag_template.substitute(**subs)


def generate_file_from_template(template_file_path: Path,
                                output_file_path: Path, **subs: str) -> None:
    ...
    Args:
        ...
        **subs: Substitutes to be applied to the template.
    """
    generated_code = generate_str_from_template(template_file_path, **subs)
